...
>
> が、もし誤った記載をしている場所がありましたらご指摘ください。本記事のGitHub上のPRでも構いません。
...
また、記者やアナウンサーなど組織に所属する個人にもA社のドメインでのアカウントを発行、運用することで、そのアカウントの身元がはっきりし、第三者によるなりすましを防ぐことができます。
メールアドレスと同じですね。
...
ここまでは既存の `.bsky.social` 上にカスタムハンドルでアカウントを運用したときと同じです。

こうなるとPDSで運用するメリットは現在はあまり感じられませんが、今後、上記図のRelaysやAppViewに相当する部分が複数運用されるようになった時、Bluesky以外の別のネットワークにも投稿や配信することができます。

この場合Blueskyが運用するAppViewの影響を受けない、分散型SNSとしてのメリットが出てきます。
...
インストールが完了すると、上記の様なメッセージが表示されます。必要な設定や管理者コマンド、ログの確認方法などが書かれているので確認しておきます。

またサーバーのサービスとして pds.service というサービスが作成されます。
...
`/pds` にディレクトリが作成されています。この中にPDSの設定やアカウントのデータが配置されます。
...
この設定は `/pds/pds.env` に以下の項目を追加することで利用できます。
...
_例:_ `PDS_EMAIL_SMTP_URL=smtps://smtpuser:password@smtp.example.com`
...
- 2024年2月現在、PDSをリレーに接続するには、公式ドキュメントからリンクされているDiscordサーバー、 AT Protocol PDS Admins に参加し、申請する必要があります。
  ...
  - また[有志の呼びかけ](https://bsky.app/profile/moja.blue/post/3km4sn7nrft24)により日本語チャンネルも設置してくれています。
    - _Nighthavenさん、きっかけをありがとうございます！_
...
- それでも表示されない場合、リバースプロキシサーバーがWebSocketでの接続に対応しているか確認してください。
  - _筆者はnginxに置き換えた際、この設定が漏れており、1日ほど費やしました…_
...
- 連合は始まりましたが、PDSはまだデベロッパー公開フェーズです。一般のユーザーを招き入れたり、アカウントを作成して楽しむ、フェーズではないようです。
- 運営からは**1つのPDSあたり10アカウント程度まで**と言われています。これを超えた場合どのような扱いになるか分かりませんが、本格的に友人知人を招待して使うには、もう少しだけ時間がかかりそうです。
...
以下のリポジトリでは、 **Cloudflare + nginx + PDS(0.4.0)** で構成しています。

設定を分けることで、1台で複数のPDSを同時に動かすこともできます。
